...
def _pop_challenge(prefix: str, user_id: int) -> bytes:
    """
    Atomic get-and-delete — a challenge is single-use, whether the ceremony
    succeeds or not. GETDEL needs Redis 6.2+; this deployment is now on
    8.0.6 (see core/redis_lock.py for the Lua-EVAL fallback this codebase
    used before the upgrade, still needed if ever pointed at an older
    Redis). A plain GET-then-DEL as two round trips would have a real, if
    narrow, race: two concurrent requests reusing the same still-valid
    challenge could both read it before either deletes it.
    """
    client = _redis_client()
